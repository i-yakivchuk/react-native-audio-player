...
//    if (self.avPlayer != nil && self.player == nil) {
//        CMTime currentTime = [[self.avPlayer currentItem] currentTime];
//        CMTime duration = [[[self.avPlayer currentItem] asset] duration];
//
//        float secondsDuration = CMTimeGetSeconds(duration);
//
//        if (secondsDuration != secondsDuration) {
//            NSArray *times = self.avPlayer.currentItem.loadedTimeRanges;
//            float total = 0;
//            for (NSValue *element in times) {
//                CMTimeRange v = element.CMTimeRangeValue;
//                float vt = CMTimeGetSeconds(v.duration);
//
//                if (vt != vt) {
//                    secondsDuration = 0;
//                } else {
...
//
//            if (times.count > 0) {
//               CMTimeRange timeRange = self.avPlayer.currentItem.loadedTimeRanges[0].CMTimeRangeValue;
//               float timeRangeDurationSeconds = CMTimeGetSeconds(timeRange.duration);
//
//               if (timeRangeDurationSeconds != timeRangeDurationSeconds) {
//                   secondsDuration = 0;
//               } else {
//                   secondsDuration = timeRangeDurationSeconds;
//               }
//            } else {
//                secondsDuration = 0;
//            }
...
//                               @"duration": [NSNumber numberWithFloat:secondsDuration]
//                               };
//        resolve(data);
//    }
}
...
- (void) prepareUrl:(NSString *)url {
    ...
    NSData *_objectData = [NSData dataWithContentsOfURL: soundURL];
    NSError *error;

    self.player = [[AVAudioPlayer alloc] initWithData:_objectData error:&error];
